...
func setUpRoundCycle(p dem.Parser, rt *RoundTracker, pe *[]posEntry, ge *[]GrenadeEntry, fe *[]FireEntry, ee *[]EventEntry, ri map[int]RoundInfo, Fp map[int][]r2.Point) {
...
func setUpPositionTracking(p dem.Parser, rt *RoundTracker, ri map[int]RoundInfo, buff *[]posEntry, batch chan []posEntry) {
				...
				if player.IsAlive() {
					...
					*buff = append(*buff, sqlEntry)
					...
					if _, ok := ri[rounds].PlayerNames[int64(player.SteamID64)]; !ok {
						ri[rounds].PlayerNames[int64(player.SteamID64)] = PlayerInfo{Name: player.Name, Side: int(player.GetTeam())}
					}
					// log.Printf("%v", player.Weapons())
					if len(ri[rounds].PlayerPositions[tick]) == 0 {
						ri[rounds].PlayerPositions[tick] = make(map[int64]PlayerState)
	...
	p.RegisterEventHandler(func(e events.RoundStart) {
		...
		DB.Exec("INSERT IGNORE INTO ROUNDS (MATCHID, ROUND_NO) VALUES (?,?)", rt.Matchid, *rt.Rounds)
		for _, players := range p.GameState().Participants().Playing() {
			DB.Exec("INSERT IGNORE INTO ROUND_PARTICIPANTS (MATCHID, ROUND_NO, PLAYERID, SIDE) VALUES (?,?,?,?)", rt.Matchid, *rt.Rounds, players.SteamID64, int(players.GetTeam()))
		}
	})
...
func setUpFireTracking(p dem.Parser, rt *RoundTracker, ri map[int]RoundInfo, Fp map[int][]r2.Point, buff *[]FireEntry, batch chan []FireEntry) {
	p.RegisterEventHandler(func(events.FrameDone) {
		if *rt.Live {
			gs := p.GameState()
			tick := gs.IngameTick()
				...
				for key, inf := range flames {
					...
					if slices.Equal(lastState, inf.Fires().Active().ConvexHull2D()) {
						...
						if len(ri[*rt.Rounds].FirePositions[tick]) == 0 {
							ri[*rt.Rounds].FirePositions[tick] = make(map[int]FireState)
						}
						if len(inf.Fires().Active().ConvexHull2D()) == 0 {
							ri[*rt.Rounds].FirePositions[tick][key] = FireState{
	...
	p.RegisterEventHandler(func(g events.InfernoStart) {
		...
		ri[*rt.Rounds].FirePositions[tick][id] = FireState{
...
func setUpEntityTracking(p dem.Parser, rt *RoundTracker, ri map[int]RoundInfo, buff *[]GrenadeEntry, batch chan []GrenadeEntry) {
	p.RegisterEventHandler(func(e events.RoundEndOfficial) {
		if !*rt.Live {
			return
		}
	...
	p.RegisterEventHandler(func(be events.BombPlanted) {
		...
		*buff = append(*buff, sqlEntry1)
		if len(ri[*rt.Rounds].GrenadeEvents[tick]) == 0 {
			ri[*rt.Rounds].GrenadeEvents[tick] = make(map[int]GrenadeState)
		}
	...
	p.RegisterEventHandler(func(be events.BombDefused) {
		...
		*buff = append(*buff, sqlEntry1)
		if len(ri[rounds].GrenadeEvents[tick]) == 0 {
			ri[rounds].GrenadeEvents[tick] = make(map[int]GrenadeState)
		}
			...
			if upTick%4 == 0 {
				...
				*buff = append(*buff, sqlEntry)
				if len(ri[*rt.Rounds].PlayerPositions[upTick]) == 0 {
					ri[*rt.Rounds].GrenadeEvents[upTick] = make(map[int]GrenadeState)
				}
...
func setUpEventTracking(p dem.Parser, rt *RoundTracker, ri map[int]RoundInfo, buff *[]EventEntry, batch chan []EventEntry) {
